OC.L10N.register(
    "terms_of_service",
    {
    "Terms of service are not signed" : "مۇلازىمەت شەرتنامىسى ئىمزالانمىدى",
    "Global" : "Global",
    "Afghanistan" : "ئافغانىستان",
    "Åland Islands" : "Åland Islands",
    "Albania" : "ئالبانىيە",
    "Algeria" : "ئالجىرىيە",
    "American Samoa" : "American Samoa",
    "Andorra" : "Andorra",
    "Angola" : "ئانگولا",
    "Anguilla" : "Anguilla",
    "Antarctica" : "ئانتاركتىكا",
    "Antigua and Barbuda" : "ئانتىگۇئا ۋە باربۇدا",
    "Argentina" : "ئارگېنتىنا",
    "Armenia" : "ئەرمىنىيە",
    "Aruba" : "Aruba",
    "Australia" : "ئاۋسترالىيە",
    "Austria" : "ئاۋسترىيە",
    "Azerbaijan" : "ئەزەربەيجان",
    "Bahamas" : "باھاما",
    "Bahrain" : "بەھرەيىن",
    "Bangladesh" : "Bangladesh",
    "Barbados" : "باربادوس",
    "Belarus" : "بېلورۇسىيە",
    "Belgium" : "بېلگىيە",
    "Belize" : "Belize",
    "Benin" : "Benin",
    "Bermuda" : "بېرمۇدا",
    "Bhutan" : "بۇتان",
    "Bolivia (Plurinational State of)" : "بولىۋىيە (Plurinational State)",
    "Bonaire, Sint Eustatius and Saba" : "Bonaire, Sint Eustatius and Saba",
    "Bosnia and Herzegovina" : "بوسنىيە-گېرتسېگوۋىنا",
    "Botswana" : "بوتسۋانا",
    "Bouvet Island" : "Bouvet Island",
    "Brazil" : "بىرازىلىيە",
    "British Indian Ocean Territory" : "ئەنگىلىيە ھىندى ئوكيان رايونى",
    "Brunei Darussalam" : "برۇنېي دارۇسسالام",
    "Bulgaria" : "بۇلغارىيە",
    "Burkina Faso" : "Burkina Faso",
    "Burundi" : "بۇرۇندى",
    "Cabo Verde" : "Cabo Verde",
    "Cambodia" : "كامبودژا",
    "Cameroon" : "كامېرون",
    "Canada" : "Canada",
    "Cayman Islands" : "كايمان ئارىلى",
    "Central African Republic" : "ئوتتۇرا ئافرىقا جۇمھۇرىيىتى",
    "Chad" : "چاد",
    "Chile" : "چىلى",
    "China" : "جۇڭگو",
    "Christmas Island" : "روژدېستۋو ئارىلى",
    "Cocos (Keeling) Islands" : "كوكوس (كېلىڭ) ئارىلى",
    "Colombia" : "كولۇمبىيە",
    "Comoros" : "Comoros",
    "Congo" : "كونگو",
    "Congo (Democratic Republic of the)" : "كونگو (دېموكراتىك جۇمھۇرىيىتى)",
    "Cook Islands" : "كۇك تاقىم ئاراللىرى",
    "Costa Rica" : "كوستارىكا",
    "Côte d'Ivoire" : "Côte d'Ivoire",
    "Croatia" : "كىرودىيە",
    "Cuba" : "كۇبا",
    "Curaçao" : "Curaçao",
    "Cyprus" : "سىپرۇس",
    "Czechia" : "چېخ",
    "Denmark" : "دانىيە",
    "Djibouti" : "جىبۇتى",
    "Dominica" : "Dominica",
    "Dominican Republic" : "دومىنىكا جۇمھۇرىيىتى",
    "Ecuador" : "ئېكۋادور",
    "Egypt" : "مىسىر",
    "El Salvador" : "سالۋادور",
    "Equatorial Guinea" : "ئېكۋاتور گۋىنىيىسى",
    "Eritrea" : "Eritrea",
    "Estonia" : "ئېستونىيە",
    "Ethiopia" : "ئېفىيوپىيە",
    "Falkland Islands (Malvinas)" : "فالكلاند ئارىلى (مالۋىناس)",
    "Faroe Islands" : "فارو ئارىلى",
    "Fiji" : "فىجى",
    "Finland" : "فىنلاندىيە",
    "France" : "France",
    "French Guiana" : "French Guiana",
    "French Polynesia" : "French Polynesia",
    "French Southern Territories" : "فرانسىيەنىڭ جەنۇبىدىكى رايونلار",
    "Gabon" : "Gabon",
    "Gambia" : "گامبىيە",
    "Georgia" : "گرۇزىيە",
    "Germany" : "Germany",
    "Ghana" : "گانا",
    "Gibraltar" : "Gibraltar",
    "Greece" : "گرېتسىيە",
    "Greenland" : "Greenland",
    "Grenada" : "Grenada",
    "Guadeloupe" : "Guadeloupe",
    "Guam" : "Guam",
    "Guatemala" : "گۋاتېمالا",
    "Guernsey" : "Guernsey",
    "Guinea" : "گۋىنىيە",
    "Guinea-Bissau" : "گۋىنىيە-بىسساۋ",
    "Guyana" : "Guyana",
    "Haiti" : "ھايتى",
    "Heard Island and McDonald Islands" : "خارد ئارىلى ۋە ماكدونالد ئارىلى",
    "Holy See" : "Holy See",
    "Honduras" : "Honduras",
    "Hong Kong" : "شياڭگاڭ",
    "Hungary" : "ۋېنگىرىيە",
    "Iceland" : "ئىسلاندىيە",
    "India" : "India",
    "Indonesia" : "ھىندونېزىيە",
    "Iran (Islamic Republic of)" : "ئىران (ئىسلام جۇمھۇرىيىتى)",
    "Iraq" : "ئىراق",
    "Ireland" : "ئىرېلاندىيە",
    "Isle of Man" : "Isle of Man",
    "Israel" : "ئىسرائىلىيە",
    "Italy" : "ئىتالىيە",
    "Jamaica" : "يامايكا",
    "Japan" : "Japan",
    "Jersey" : "Jersey",
    "Jordan" : "ئىئوردانىيە",
    "Kazakhstan" : "قازاقىستان",
    "Kenya" : "كېنىيە",
    "Kiribati" : "Kiribati",
    "Korea (Democratic People's Republic of)" : "كورېيە (دېموكراتىك خەلق جۇمھۇرىيىتى)",
    "Korea (Republic of)" : "كورېيە (جۇمھۇرىيىتى)",
    "Kuwait" : "كۇۋەيت",
    "Kyrgyzstan" : "قىرغىزىستان",
    "Lao People's Democratic Republic" : "لائوس خەلق دېموكراتىك جۇمھۇرىيىتى",
    "Latvia" : "Latvia",
    "Lebanon" : "لىۋان",
    "Lesotho" : "Lesotho",
    "Liberia" : "Liberia",
    "Libya" : "لىۋىيە",
    "Liechtenstein" : "Liechtenstein",
    "Lithuania" : "لىتۋا",
    "Luxembourg" : "لىيۇكسېمبۇرگ",
    "Macao" : "ئاۋمېن",
    "Macedonia (the former Yugoslav Republic of)" : "ماكېدونىيە (سابىق يۇگوسلاۋىيە جۇمھۇرىيىتى)",
    "Madagascar" : "ماداغاسقار",
    "Malawi" : "مالاۋى",
    "Malaysia" : "مالايسىيا",
    "Maldives" : "مالدىۋى",
    "Mali" : "مالى",
    "Malta" : "مالتا",
    "Marshall Islands" : "مارشال تاقىم ئاراللىرى",
    "Martinique" : "Martinique",
    "Mauritania" : "ماۋرىتانىيە",
    "Mauritius" : "ماۋرىتىئۇس",
    "Mayotte" : "مايوت",
    "Mexico" : "Mexico",
    "Micronesia (Federated States of)" : "مىكرونېزىيە (فېدېراتسىيە شىتاتلىرى)",
    "Moldova (Republic of)" : "مولدوۋا (جۇمھۇرىيىتى)",
    "Monaco" : "موناكو",
    "Mongolia" : "موڭغۇلىيە",
    "Montenegro" : "قاراتاغ",
    "Montserrat" : "Montserrat",
    "Morocco" : "ماراكەش",
    "Mozambique" : "Mozambique",
    "Myanmar" : "بېرما",
    "Namibia" : "نامبىيە",
    "Nauru" : "Nauru",
    "Nepal" : "Nepal",
    "Netherlands" : "گوللاندىيە",
    "New Caledonia" : "يېڭى كالېدونىيە",
    "New Zealand" : "يېڭى زېلاندىيە",
    "Nicaragua" : "نىكاراگۇئا",
    "Niger" : "Niger",
    "Nigeria" : "نىگېرىيە",
    "Niue" : "Niue",
    "Norfolk Island" : "نورفولك ئارىلى",
    "Northern Mariana Islands" : "شىمالىي مارىئانا ئارىلى",
    "Norway" : "نورۋېگىيە",
    "Oman" : "ئومان",
    "Pakistan" : "Pakistan",
    "Palau" : "Palau",
    "Palestine, State of" : "پەلەستىن ، شىتات",
    "Panama" : "پاناما",
    "Papua New Guinea" : "پاپۇئا يېڭى گۋىنىيىسى",
    "Paraguay" : "پاراگۋاي",
    "Peru" : "پېرۇ",
    "Philippines" : "فىلىپپىن",
    "Pitcairn" : "Pitcairn",
    "Poland" : "پولشا",
    "Portugal" : "Portugal",
    "Puerto Rico" : "پورتو رىكو",
    "Qatar" : "قاتار",
    "Réunion" : "Réunion",
    "Romania" : "رۇمىنىيە",
    "Russian Federation" : "رۇسىيە فېدېراتسىيەسى",
    "Rwanda" : "رىۋاندا",
    "Saint Barthélemy" : "Saint Barthélemy",
    "Saint Helena, Ascension and Tristan da Cunha" : "Saint Helena, Ascension and Tristan da Cunha",
    "Saint Kitts and Nevis" : "Saint Kitts and Nevis",
    "Saint Lucia" : "Saint Lucia",
    "Saint Martin (French part)" : "سانت مارتىن (فىرانسۇزچە قىسمى)",
    "Saint Pierre and Miquelon" : "Saint Pierre and Miquelon",
    "Saint Vincent and the Grenadines" : "ئەۋلىيا ۋىنسېنت ۋە گرېنادىنلار",
    "Samoa" : "ساموئا",
    "San Marino" : "San Marino",
    "Sao Tome and Principe" : "سان توم ۋە پرىنسىپ",
    "Saudi Arabia" : "سەئۇدى ئەرەبىستان",
    "Senegal" : "سېنېگال",
    "Serbia" : "سېربىيە",
    "Seychelles" : "Seychelles",
    "Sierra Leone" : "سېررالېئون",
    "Singapore" : "سىنگاپور",
    "Sint Maarten (Dutch part)" : "سىنت مارتىن (گوللاندىيە قىسمى)",
    "Slovakia" : "سلوۋاكىيە",
    "Slovenia" : "سىلوۋېنىيە",
    "Solomon Islands" : "سۇلايمان تاقىم ئاراللىرى",
    "Somalia" : "سومالى",
    "South Africa" : "جەنۇبى ئافرىقا",
    "South Georgia and the South Sandwich Islands" : "جەنۇبىي گرۇزىيە ۋە جەنۇبىي ساندۋىچ ئارىلى",
    "South Sudan" : "جەنۇبىي سۇدان",
    "Spain" : "ئىسپانىيە",
    "Sri Lanka" : "سىرىلانكا",
    "Sudan" : "سۇدان",
    "Suriname" : "Suriname",
    "Svalbard and Jan Mayen" : "سۋالبارد ۋە يان مايېن",
    "Swaziland" : "Swaziland",
    "Sweden" : "شىۋىتسىيە",
    "Switzerland" : "شىۋىتسارىيە",
    "Syrian Arab Republic" : "سۈرىيە ئەرەب جۇمھۇرىيىتى",
    "Taiwan" : "تەيۋەن",
    "Tajikistan" : "تاجىكىستان",
    "Tanzania, United Republic of" : "تانزانىيە ، بىرلەشمە جۇمھۇرىيىتى",
    "Thailand" : "تايلاند",
    "Timor-Leste" : "Timor-Leste",
    "Togo" : "توگو",
    "Tokelau" : "Tokelau",
    "Tonga" : "تونگا",
    "Trinidad and Tobago" : "تىرىنىداد ۋە توباگو",
    "Tunisia" : "تۇنىس",
    "Turkey" : "تۈركىيە",
    "Turkmenistan" : "تۈركمەنىستان",
    "Turks and Caicos Islands" : "تۈركلەر ۋە كايكوس ئارىلى",
    "Tuvalu" : "Tuvalu",
    "Uganda" : "ئۇگاندا",
    "Ukraine" : "ئۇكرائىنا",
    "United Arab Emirates" : "ئەرەب بىرلەشمە خەلىپىلىكى",
    "United Kingdom of Great Britain and Northern Ireland" : "بۈيۈك بىرىتانىيە ۋە شىمالىي ئىرېلاندىيە",
    "United States of America" : "ئامېرىكا قوشما شتاتلىرى",
    "United States Minor Outlying Islands" : "ئامېرىكا كىچىك تاشقى ئاراللار",
    "Uruguay" : "ئۇرۇگۋاي",
    "Uzbekistan" : "ئۆزبېكىستان",
    "Vanuatu" : "Vanuatu",
    "Venezuela (Bolivarian Republic of)" : "ۋېنېزۇئېلا (بولىۋىيە جۇمھۇرىيىتى)",
    "Viet Nam" : "Vietnam Nam",
    "Virgin Islands (British)" : "ۋىرگىن تاقىم ئارىلى (ئەنگىلىيە)",
    "Virgin Islands (U.S.)" : "ۋىرگىنىيە تاقىم ئارىلى (ئامېرىكا)",
    "Wallis and Futuna" : "ۋاللىس ۋە فۇتۇنا",
    "Western Sahara" : "غەربىي سەھرايى كەبىر",
    "Yemen" : "يەمەن",
    "Zambia" : "زامبىيە",
    "Zimbabwe" : "زىمبابۋې",
    "Afar" : "Afar",
    "Abkhazian" : "Abkhazian",
    "Avestan" : "Avestan",
    "Afrikaans" : "ئافرىقا",
    "Akan" : "Akan",
    "Amharic" : "Amharic",
    "Aragonese" : "Aragonese",
    "Arabic" : "ئەرەبچە",
    "Assamese" : "Assamese",
    "Avaric" : "Avaric",
    "Aymara" : "Aymara",
    "Azerbaijani" : "ئەزەربەيجان",
    "Bashkir" : "بەشىر",
    "Belarusian" : "بېلورۇسىيە",
    "Bulgarian" : "بۇلغارىيە",
    "Bihari languages" : "بىھارى تىلى",
    "Bislama" : "Bislama",
    "Bambara" : "Bambara",
    "Bengali" : "Bengali",
    "Tibetan" : "زاڭزۇ",
    "Breton" : "Breton",
    "Bosnian" : "Bosnian",
    "Catalan; Valencian" : "كاتالونىيە Valencian",
    "Chechen" : "چېچەن",
    "Chamorro" : "Chamorro",
    "Corsican" : "كورسىكان",
    "Cree" : "Cree",
    "Czech" : "چېخ",
    "Church Slavic; Old Slavonic; Church Slavonic; Old Bulgarian; Old Church Slavonic" : "چېركاۋ سىلاۋىيان كونا سىلاۋيان تىلى چېركاۋ سىلاۋونىك كونا بۇلغارىيە كونا چېركاۋ سىلاۋيان",
    "Chuvash" : "Chuvash",
    "Welsh" : "Welsh",
    "Danish" : "Danish",
    "German" : "German",
    "Divehi; Dhivehi; Maldivian" : "Divehi; Dhivehi; Maldivian",
    "Dzongkha" : "Dzongkha",
    "Ewe" : "Ewe",
    "Greek, Modern (1453-)" : "گرېتسىيە ، زامانىۋى (1453-)",
    "English" : "ئىنگىلىزچە",
    "Esperanto" : "Esperanto",
    "Spanish; Castilian" : "ئىسپانچە Castilian",
    "Estonian" : "ئېستونىيە",
    "Basque" : "Basque",
    "Persian" : "پارسچە",
    "Fulah" : "Fulah",
    "Finnish" : "فىنلاندىيە",
    "Fijian" : "Fijian",
    "Faroese" : "Faroese",
    "French" : "فىرانسۇزچە",
    "Western Frisian" : "Western Frisian",
    "Irish" : "ئىرېلاندىيە",
    "Gaelic; Scottish Gaelic" : "گال تىلى شوتلاندىيە گال تىلى",
    "Galician" : "Galician",
    "Guarani" : "Guarani",
    "Gujarati" : "Gujarati",
    "Manx" : "Manx",
    "Hausa" : "Hausa",
    "Hebrew" : "ئىبرانىي تىلى",
    "Hindi" : "Hindi",
    "Hiri Motu" : "Hiri Motu",
    "Croatian" : "كىرودىيە",
    "Haitian; Haitian Creole" : "ھايتى ھايتى كرېئول",
    "Hungarian" : "ۋېنگرىيە",
    "Armenian" : "ئەرمىنىيە",
    "Herero" : "Herero",
    "Interlingua (International Auxiliary Language Association)" : "Interlingua (خەلقئارا ياردەمچى تىل جەمئىيىتى)",
    "Indonesian" : "ھىندونېزىيە",
    "Interlingue; Occidental" : "Interlingue; تاسادىپىي",
    "Igbo" : "Igbo",
    "Sichuan Yi; Nuosu" : "سىچۈەن يى; Nuosu",
    "Inupiaq" : "Inupiaq",
    "Ido" : "Ido",
    "Icelandic" : "ئىسلاندىيە",
    "Italian" : "Italian",
    "Inuktitut" : "Inuktitut",
    "Japanese" : "ياپون",
    "Javanese" : "Javanese",
    "Georgian" : "گرۇزىيە",
    "Kongo" : "كونگو",
    "Kikuyu; Gikuyu" : "Kikuyu; Gikuyu",
    "Kuanyama; Kwanyama" : "Kuanyama; Kwanyama",
    "Kazakh" : "قازاق",
    "Kalaallisut; Greenlandic" : "Kalaallisut; Greenlandic",
    "Central Khmer" : "Central Khmer",
    "Kannada" : "Kannada",
    "Korean" : "كورېيە",
    "Kanuri" : "Kanuri",
    "Kashmiri" : "كەشمىر",
    "Kurdish" : "كۇرد",
    "Komi" : "كومى",
    "Cornish" : "قوناق",
    "Kirghiz; Kyrgyz" : "قىرغىز; قىرغىزىستان",
    "Latin" : "لاتىنچە",
    "Luxembourgish; Lëtzeburgesch" : "لىيۇكسېمبۇرگ Lëtzeburgesch",
    "Ganda" : "Ganda",
    "Limburgan; Limburger; Limburgish" : "Limburgan; Limburger; Limburgish",
    "Lingala" : "Lingala",
    "Lao" : "لائوس",
    "Lithuanian" : "لىتۋانىيە",
    "Luba-Katanga" : "Luba-Katanga",
    "Latvian" : "Latvian",
    "Malagasy" : "Malagasy",
    "Marshallese" : "Marshallese",
    "Maori" : "Maori",
    "Macedonian" : "ماكېدونىيە",
    "Malayalam" : "Malayalam",
    "Mongolian" : "موڭغۇل",
    "Marathi" : "Marathi",
    "Malay" : "مالايسىيا",
    "Maltese" : "Maltese",
    "Burmese" : "بېرما",
    "Bokmål, Norwegian; Norwegian Bokmål" : "بوكمىل ، نورۋېگچە نورۋېگىيە بوكمىل",
    "Ndebele, North; North Ndebele" : "Ndebele, North; North Ndebele",
    "Nepali" : "نېپال",
    "Ndonga" : "Ndonga",
    "Dutch; Flemish" : "گوللاندىيەچە Flemish",
    "Norwegian Nynorsk; Nynorsk, Norwegian" : "نورۋېگىيە نىنورسك Nynorsk, Norwegian",
    "Norwegian" : "نورۋېگچە",
    "Ndebele, South; South Ndebele" : "Ndebele, South; South Ndebele",
    "Navajo; Navaho" : "ناۋاجو; Navaho",
    "Chichewa; Chewa; Nyanja" : "چىچېۋا Chewa; Nyanja",
    "Occitan (post 1500)" : "ئوكسىتان (1500-نومۇرلۇق يازما)",
    "Ojibwa" : "Ojibwa",
    "Oromo" : "Oromo",
    "Oriya" : "Oriya",
    "Ossetian; Ossetic" : "Ossetian; Ossetic",
    "Panjabi; Punjabi" : "Panjabi; Punjabi",
    "Pali" : "Pali",
    "Polish" : "پولشا",
    "Pushto; Pashto" : "Pushto; پۇشتۇ",
    "Portuguese" : "پورتۇگال تىلى",
    "Quechua" : "Quechua",
    "Romansh" : "Romansh",
    "Rundi" : "Rundi",
    "Romanian; Moldavian; Moldovan" : "رومان تىلى مولداۋىيان مولدوۋان",
    "Russian" : "رۇسچە",
    "Kinyarwanda" : "Kinyarwanda",
    "Sanskrit" : "سانسكرىت تىلى",
    "Sardinian" : "ساردىيان",
    "Sindhi" : "سىندى",
    "Northern Sami" : "شىمالىي سامى",
    "Sango" : "Sango",
    "Sinhala; Sinhalese" : "Sinhala; Sinhalese",
    "Slovak" : "سلوۋاكىيە",
    "Slovenian" : "سىلوۋېنىيە",
    "Samoan" : "Samoan",
    "Shona" : "Shona",
    "Somali" : "سومالى",
    "Albanian" : "ئالبانىيە",
    "Serbian" : "سېربىيە",
    "Swati" : "Swati",
    "Sotho, Southern" : "Sotho, Southern",
    "Sundanese" : "Sundanese",
    "Swedish" : "شىۋىتسىيە",
    "Swahili" : "Swahili",
    "Tamil" : "تامىل",
    "Telugu" : "Telugu",
    "Tajik" : "تاجىك",
    "Thai" : "تايلاند",
    "Tigrinya" : "Tigrinya",
    "Turkmen" : "تۈركمەنلەر",
    "Tagalog" : "Tagalog",
    "Tswana" : "Tswana",
    "Tonga (Tonga Islands)" : "تونگا (تونگا تاقىم ئاراللىرى)",
    "Turkish" : "تۈرك",
    "Tsonga" : "Tsonga",
    "Tatar" : "تاتار",
    "Twi" : "Twi",
    "Tahitian" : "Tahitian",
    "Uighur; Uyghur" : "ئۇيغۇر ئۇيغۇر",
    "Ukrainian" : "ئۇكرائىنا",
    "Urdu" : "ئوردۇچە",
    "Uzbek" : "ئۆزبېك",
    "Venda" : "Venda",
    "Vietnamese" : "ۋېيتنام",
    "Volapük" : "Volapük",
    "Walloon" : "ۋاللون",
    "Wolof" : "Wolof",
    "Xhosa" : "Xhosa",
    "Yiddish" : "Yiddish",
    "Yoruba" : "يورۇبا",
    "Zhuang; Chuang" : "جۇاڭ چۇاڭ",
    "Chinese" : "خەنزۇچە",
    "Zulu" : "Zulu",
    "Terms of service" : "مۇلازىمەت شەرتلىرى",
    "Terms of service have been modified!" : "مۇلازىمەت شەرتلىرى ئۆزگەرتىلدى!",
    "You have to accept the newest version of the terms of service in order to be able to use this service." : "بۇ مۇلازىمەتنى ئىشلىتىش ئۈچۈن ئەڭ يېڭى نەشرىدىكى مۇلازىمەت شەرتلىرىنى قوبۇل قىلىشىڭىز كېرەك.",
    "Requires users to accept the terms of service before accessing data." : "ئابونتلارنىڭ سانلىق مەلۇماتقا ئېرىشىشتىن بۇرۇن مۇلازىمەت شەرتلىرىنى قوبۇل قىلىشىنى تەلەپ قىلىدۇ.",
    "Requires users to accept the terms of service before accessing data.\nThis product includes GeoLite2 data created by MaxMind, available from [maxmind.com](http://www.maxmind.com)" : "ئابونتلارنىڭ سانلىق مەلۇماتقا ئېرىشىشتىن بۇرۇن مۇلازىمەت شەرتلىرىنى قوبۇل قىلىشىنى تەلەپ قىلىدۇ.\nبۇ مەھسۇلات MaxMind قۇرغان GeoLite2 سانلىق مەلۇماتلىرىنى ئۆز ئىچىگە ئالىدۇ ، [maxmind.com] دىن كەلگەن (http://www.maxmind.com)",
    "Require users to accept the terms of service before accessing the service." : "ئىشلەتكۈچىلەرنىڭ مۇلازىمەتكە كىرىشتىن بۇرۇن مۇلازىمەت شەرتلىرىنى قوبۇل قىلىشىنى تەلەپ قىلىڭ.",
    "Show for logged-in users" : "تىزىملاتقان ئىشلەتكۈچىلەرنى كۆرسىتىش",
    "Show on public shares" : "ئاممىۋى پاينى كۆرسىتىش",
    "Select a region" : "رايون تاللاڭ",
    "Select a language" : "تىل تاللاڭ",
    "By using this service …" : "بۇ مۇلازىمەتنى ئىشلىتىش ئارقىلىق…",
    "For formatting purposes Markdown is supported." : "فورماتلاش مەقسىتىدە Markdown نى قوللايدۇ.",
    "Existing terms of service" : "ھازىر بار بولغان مۇلازىمەت شەرتلىرى",
    "Loading …" : "Loading…",
    "Ensure that all fields are filled" : "ھەممە ساھەنىڭ تولدۇرۇلۇشىغا كاپالەتلىك قىلىڭ",
    "Terms saved successfully!" : "شەرتلەر مۇۋەپپەقىيەتلىك ساقلاندى!",
    "All signatories reset!" : "بارلىق ئىمزا قويغۇچىلار ئەسلىگە كەلدى!",
    "I acknowledge that I have read and agree to the above terms of service" : "مەن يۇقىرىدىكى مۇلازىمەت شەرتلىرىنى ئوقۇغانلىقىم ۋە ئۇنىڭغا قوشۇلغانلىقىمنى ئېتىراپ قىلىمەن",
    "Edit language {language} for region {country}" : "رايون {دۆلەت ئۈچۈن تىل {تىل} تەھرىرلەش",
    "Deleting …" : "ئۆچۈرۈش…",
    "Delete language {language} for region {country}" : "رايون {دۆلەت for ئۈچۈن تىل {تىل ئۆچۈرۈڭ"
},
"nplurals=2; plural=(n != 1);");
