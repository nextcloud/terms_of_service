OC.L10N.register(
    "terms_of_service",
    {
    "Global" : "Καθολικό",
    "Afghanistan" : "Αφγανιστάν",
    "Åland Islands" : "Åland Islands",
    "Albania" : "Αλβανία",
    "Algeria" : "Αλγερία",
    "American Samoa" : "Αμερικανική Σαμόα",
    "Andorra" : "Ανδόρα",
    "Angola" : "Αγκόλα",
    "Anguilla" : "Anguilla",
    "Antarctica" : "Ανταρκτική",
    "Antigua and Barbuda" : "Αντίκουα και Μπαρμπούντα",
    "Argentina" : "Αργεντινή",
    "Armenia" : "Αρμενία",
    "Aruba" : "Aruba",
    "Australia" : "Αυστραλία",
    "Austria" : "Αυστρία",
    "Azerbaijan" : "Αζερμπαϊτζάν",
    "Bahamas" : "Μπαχάμες",
    "Bahrain" : "Μπαχρέιν",
    "Bangladesh" : "Μπανγκλαντές",
    "Barbados" : "Μπαρμπάντος",
    "Belarus" : "Λευκορωσία",
    "Belgium" : "Βέλγιο",
    "Belize" : "Μπελίσε",
    "Benin" : "Μπενίν",
    "Bermuda" : "Βερμούδα",
    "Bhutan" : "Μπουτάν",
    "Bolivia (Plurinational State of)" : "Bolivia (Plurinational State of)",
    "Bonaire, Sint Eustatius and Saba" : "Bonaire, Sint Eustatius and Saba",
    "Bosnia and Herzegovina" : "Βοσνία και Ερζεγοβίνη",
    "Botswana" : "Μποτσουάνα",
    "Bouvet Island" : "Bouvet Island",
    "Brazil" : "Βραζιλία",
    "British Indian Ocean Territory" : "British Indian Ocean Territory",
    "Brunei Darussalam" : "Brunei Darussalam",
    "Bulgaria" : "Βουλγαρία",
    "Burkina Faso" : "Μπουρκίνα Φάσο",
    "Burundi" : "Μπουρούντι",
    "Cabo Verde" : "Cabo Verde",
    "Cambodia" : "Cambodia",
    "Cameroon" : "Καμερούν",
    "Canada" : "Καναδάς",
    "Cayman Islands" : "Cayman Islands",
    "Central African Republic" : "Δημοκρατία Κεντρικής Αφρικής",
    "Chad" : "Chad",
    "Chile" : "Χιλή",
    "China" : "Κίνα",
    "Christmas Island" : "Νήσοι Χριστουγέννων",
    "Cocos (Keeling) Islands" : "Cocos (Keeling) Islands",
    "Colombia" : "Κολομβία",
    "Comoros" : "Comoros",
    "Congo" : "Κονγκό",
    "Congo (Democratic Republic of the)" : "Congo (Democratic Republic of the)",
    "Cook Islands" : "Cook Islands",
    "Costa Rica" : "Costa Rica",
    "Côte d'Ivoire" : "Côte d'Ivoire",
    "Croatia" : "Κροατία",
    "Cuba" : "Κούβα",
    "Curaçao" : "Curaçao",
    "Cyprus" : "Κύπρος",
    "Czechia" : "Czechia",
    "Denmark" : "Δανία",
    "Djibouti" : "Djibouti",
    "Dominica" : "Dominica",
    "Dominican Republic" : "Dominican Republic",
    "Ecuador" : "Εκουαδόρ",
    "Egypt" : "Αίγυπτος",
    "El Salvador" : "Ελ Σαλβαδόρ",
    "Equatorial Guinea" : "Equatorial Guinea",
    "Eritrea" : "Eritrea",
    "Estonia" : "Εσθονία",
    "Ethiopia" : "Αιθιοπία",
    "Falkland Islands (Malvinas)" : "Falkland Islands (Malvinas)",
    "Faroe Islands" : "Νησιά Φερόε",
    "Fiji" : "Φίτζι",
    "Finland" : "Φινλανδία",
    "France" : "Γαλλία",
    "French Guiana" : "French Guiana",
    "French Polynesia" : "Γαλλική Πολυνησία",
    "French Southern Territories" : "Γαλλικά νότια εδάφη",
    "Gabon" : "Gabon",
    "Gambia" : "Γκάμπια",
    "Georgia" : "Georgia",
    "Germany" : "Γερμανία",
    "Ghana" : "Ghana",
    "Gibraltar" : "Gibraltar",
    "Greece" : "Ελλάδα",
    "Greenland" : "Greenland",
    "Grenada" : "Γρενάδα",
    "Guadeloupe" : "Γουαδελούπη",
    "Guam" : "Guam",
    "Guatemala" : "Γουατεμάλα",
    "Guernsey" : "Guernsey",
    "Guinea" : "Γουϊνέα",
    "Guinea-Bissau" : "Guinea-Bissau",
    "Guyana" : "Guyana",
    "Haiti" : "Haiti",
    "Heard Island and McDonald Islands" : "Heard Island and McDonald Islands",
    "Holy See" : "Holy See",
    "Honduras" : "Honduras",
    "Hong Kong" : "Hong Kong",
    "Hungary" : "Ουγγαρία",
    "Iceland" : "Ισλανδία",
    "India" : "Ινδία",
    "Indonesia" : "Ινδονησία",
    "Iran (Islamic Republic of)" : "Ιράν (Ισλαμική Δημοκρατία)",
    "Iraq" : "Ιράκ",
    "Ireland" : "Ιρλανδία",
    "Isle of Man" : "Isle of Man",
    "Israel" : "Ισραήλ",
    "Italy" : "Ιταλία",
    "Jamaica" : "Τζαμάικα",
    "Japan" : "Ιαπωνία",
    "Jersey" : "Jersey",
    "Jordan" : "Ιορδανία",
    "Kazakhstan" : "Καζακστάν",
    "Kenya" : "Κένυα",
    "Kiribati" : "Kiribati",
    "Korea (Democratic People's Republic of)" : "Κορέα (Λαϊκή Δημοκρατία της)",
    "Korea (Republic of)" : "Κορέα (Δημοκρατία της)",
    "Kuwait" : "Κουβέιτ",
    "Kyrgyzstan" : "Kyrgyzstan",
    "Lao People's Democratic Republic" : "Λαϊκή Δημοκρατία του Λάος",
    "Latvia" : "Λετονία",
    "Lebanon" : "Λίβανος",
    "Lesotho" : "Λεσότο",
    "Liberia" : "Λιβερία",
    "Libya" : "Λιβύη",
    "Liechtenstein" : "Λιχτενστάιν",
    "Lithuania" : "Λιθουανία",
    "Luxembourg" : "Λουξεμβούργο",
    "Macao" : "Μακάο",
    "Madagascar" : "Μαδαγασκάρη",
    "Malawi" : "Μαλάουι",
    "Malaysia" : "Μαλαισία",
    "Maldives" : "Μαλδίβες",
    "Mali" : "Μάλι",
    "Malta" : "Μάλτα",
    "Marshall Islands" : "Marshall Islands",
    "Martinique" : "Μαρτινίκα",
    "Mauritania" : "Μαυριτανία",
    "Mauritius" : "Μαυρίκιος",
    "Mayotte" : "Mayotte",
    "Mexico" : "Μεξικό",
    "Micronesia (Federated States of)" : "Micronesia (Federated States of)",
    "Moldova (Republic of)" : "Moldova (Republic of)",
    "Monaco" : "Μονακό",
    "Mongolia" : "Μογγολία",
    "Montenegro" : "Μαυροβούνιο",
    "Montserrat" : "Montserrat",
    "Morocco" : "Μαρόκο",
    "Mozambique" : "Μοζαμβίκη",
    "Myanmar" : "Myanmar",
    "Namibia" : "Ναμίμπια",
    "Nauru" : "Nauru",
    "Nepal" : "Νεπάλ",
    "Netherlands" : "Ολλανδία",
    "New Caledonia" : "New Caledonia",
    "New Zealand" : "Νέα Ζηλανδία",
    "Nicaragua" : "Νικαράγουα",
    "Niger" : "Νίγηρας",
    "Nigeria" : "Νιγηρία",
    "Niue" : "Niue",
    "Norfolk Island" : "Norfolk Island",
    "North Macedonia" : "Δημοκρατία της Βόρειας Μακεδονίας",
    "Northern Mariana Islands" : "Northern Mariana Islands",
    "Norway" : "Νορβηγία",
    "Oman" : "Ομάν",
    "Pakistan" : "Πακιστάν",
    "Palau" : "Palau",
    "Palestine, State of" : "Παλαιστίνη, Κράτος της",
    "Panama" : "Παναμάς",
    "Papua New Guinea" : "Παπούα Νέα Γουινέα",
    "Paraguay" : "Παραγουάη",
    "Peru" : "Περού",
    "Philippines" : "Φιλιππίνες",
    "Pitcairn" : "Pitcairn",
    "Poland" : "Πολωνία",
    "Portugal" : "Πορτογαλία",
    "Puerto Rico" : "Πόρτο Ρίκο",
    "Qatar" : "Κατάρ",
    "Réunion" : "Réunion",
    "Romania" : "Ρουμανία",
    "Russian Federation" : "Russian Federation",
    "Rwanda" : "Ρουάντα",
    "Saint Barthélemy" : "Saint Barthélemy",
    "Saint Helena, Ascension and Tristan da Cunha" : "Saint Helena, Ascension and Tristan da Cunha",
    "Saint Kitts and Nevis" : "Saint Kitts and Nevis",
    "Saint Lucia" : "Saint Lucia",
    "Saint Martin (French part)" : "Saint Martin (French part)",
    "Saint Pierre and Miquelon" : "Saint Pierre and Miquelon",
    "Saint Vincent and the Grenadines" : "Saint Vincent and the Grenadines",
    "Samoa" : "Σαμόα",
    "San Marino" : "Σαν Μαρίνο",
    "Sao Tome and Principe" : "Sao Tome and Principe",
    "Saudi Arabia" : "Σαουδική Αραβία",
    "Senegal" : "Σενεγάλη",
    "Serbia" : "Σερβία",
    "Seychelles" : "Σεϊχέλες",
    "Sierra Leone" : "Σιέρα Λεόνε",
    "Singapore" : "Σιγκαπούρη",
    "Sint Maarten (Dutch part)" : "Sint Maarten (Dutch part)",
    "Slovakia" : "Σλοβακία",
    "Slovenia" : "Σλοβενία",
    "Solomon Islands" : "Νησιά Σολομώντα",
    "Somalia" : "Σομαλία",
    "South Africa" : "Νότιος Αφρική",
    "South Georgia and the South Sandwich Islands" : "South Georgia and the South Sandwich Islands",
    "South Sudan" : "Νότιο Σουδάν",
    "Spain" : "Ισπανία",
    "Sri Lanka" : "Σρι Λάνκα",
    "Sudan" : "Σουδάν",
    "Suriname" : "Σουρινάμ",
    "Svalbard and Jan Mayen" : "Svalbard and Jan Mayen",
    "Swaziland" : "Σουαζιλάνδη",
    "Sweden" : "Σουηδία",
    "Switzerland" : "Ελβετία",
    "Syrian Arab Republic" : "Syrian Arab Republic",
    "Taiwan" : "Ταϊβάν",
    "Tajikistan" : "Tajikistan",
    "Tanzania, United Republic of" : "Tanzania, United Republic of",
    "Thailand" : "Ταϊλάνδη",
    "Timor-Leste" : "Timor-Leste",
    "Togo" : "Togo",
    "Tokelau" : "Tokelau",
    "Tonga" : "Tonga",
    "Trinidad and Tobago" : "Trinidad and Tobago",
    "Tunisia" : "Τυνησία",
    "Turkey" : "Τουρκία",
    "Turkmenistan" : "Τουρκμενιστάν",
    "Turks and Caicos Islands" : "Turks and Caicos Islands",
    "Tuvalu" : "Τουβάλου",
    "Uganda" : "Ουγκάντα",
    "Ukraine" : "Ουκρανία",
    "United Arab Emirates" : "Ηνωμένα Αραβικά Εμιράτα",
    "United Kingdom of Great Britain and Northern Ireland" : "Ηνωμένο Βασίλειο της Μεγάλης Βρετανίας και Βορείου Ιρλανδίας",
    "United States of America" : "United States of America",
    "United States Minor Outlying Islands" : "United States Minor Outlying Islands",
    "Uruguay" : "Ουρουγουάη",
    "Uzbekistan" : "Uzbekistan",
    "Vanuatu" : "Βανουάτου",
    "Venezuela (Bolivarian Republic of)" : "Venezuela (Bolivarian Republic of)",
    "Viet Nam" : "Βιετνάμ",
    "Virgin Islands (British)" : "Παρθένοι Νήσοι (Βρετανία)",
    "Virgin Islands (U.S.)" : "Παρθένοι Νήσοι (ΗΠΑ)",
    "Wallis and Futuna" : "Wallis and Futuna",
    "Western Sahara" : "Δυτική Σαχάρα",
    "Yemen" : "Υεμένη",
    "Zambia" : "Ζάμπια",
    "Zimbabwe" : "Ζιμπάμπουε",
    "Afar" : "Afar",
    "Abkhazian" : "Abkhazian",
    "Avestan" : "Avestan",
    "Afrikaans" : "Αφρικανικά",
    "Akan" : "Ακάν",
    "Amharic" : "Αμχαρικά",
    "Aragonese" : "Aragonese",
    "Arabic" : "Αραβικά",
    "Assamese" : "Ασσαμέζικα",
    "Avaric" : "Avaric",
    "Aymara" : "Aymara",
    "Azerbaijani" : "Αζερμπαϊτζανικά",
    "Bashkir" : "Bashkir",
    "Belarusian" : "Belarusian",
    "Bulgarian" : "Βούλγαρος",
    "Bihari languages" : "Bihari languages",
    "Bislama" : "Bislama",
    "Bambara" : "Bambara",
    "Bengali" : "Bengali",
    "Tibetan" : "Tibetan",
    "Breton" : "Breton",
    "Bosnian" : "Bosnian",
    "Catalan; Valencian" : "Catalan; Valencian",
    "Chechen" : "Chechen",
    "Chamorro" : "Chamorro",
    "Corsican" : "Κορσικά",
    "Cree" : "Cree",
    "Czech" : "Τσέχικα",
    "Church Slavic; Old Slavonic; Church Slavonic; Old Bulgarian; Old Church Slavonic" : "Church Slavic; Old Slavonic; Church Slavonic; Old Bulgarian; Old Church Slavonic",
    "Chuvash" : "Chuvash",
    "Welsh" : "Welsh",
    "Danish" : "Δανέζικα",
    "German" : "Γερμανικά",
    "Divehi; Dhivehi; Maldivian" : "Divehi; Dhivehi; Maldivian",
    "Dzongkha" : "Dzongkha",
    "Ewe" : "Ewe",
    "Greek, Modern (1453-)" : "Ελληνικά, Μοντέρνα (1453-)",
    "English" : "Αγγλικά",
    "Esperanto" : "Εσπεράντο",
    "Spanish; Castilian" : "Spanish; Castilian",
    "Estonian" : "Εσθονικά",
    "Basque" : "Βασικικά",
    "Persian" : "Περσικά",
    "Fulah" : "Fulah",
    "Finnish" : "Φινλανδικά",
    "Fijian" : "Fijian",
    "Faroese" : "Faroese",
    "French" : "Γαλλικά",
    "Western Frisian" : "Western Frisian",
    "Irish" : "Irish",
    "Gaelic; Scottish Gaelic" : "Gaelic; Scottish Gaelic",
    "Galician" : "Galician",
    "Guarani" : "Guarani",
    "Gujarati" : "Gujarati",
    "Manx" : "Manx",
    "Hausa" : "Hausa",
    "Hebrew" : "Εβραϊκά",
    "Hindi" : "Hindi",
    "Hiri Motu" : "Hiri Motu",
    "Croatian" : "Κροατικά",
    "Haitian; Haitian Creole" : "Haitian; Haitian Creole",
    "Hungarian" : "Ουγγρικά",
    "Armenian" : "Αρμένικα",
    "Herero" : "Herero",
    "Interlingua (International Auxiliary Language Association)" : "Interlingua (International Auxiliary Language Association)",
    "Indonesian" : "Ινδονησιακά",
    "Interlingue; Occidental" : "Interlingue; Occidental",
    "Igbo" : "Igbo",
    "Sichuan Yi; Nuosu" : "Sichuan Yi; Nuosu",
    "Inupiaq" : "Inupiaq",
    "Ido" : "Ido",
    "Icelandic" : "Ισλανδικά",
    "Italian" : "Ιταλικά",
    "Inuktitut" : "Inuktitut",
    "Japanese" : "Japanese",
    "Javanese" : "Javanese",
    "Georgian" : "Georgian",
    "Kongo" : "Kongo",
    "Kikuyu; Gikuyu" : "Kikuyu; Gikuyu",
    "Kuanyama; Kwanyama" : "Kuanyama; Kwanyama",
    "Kazakh" : "Kazakh",
    "Kalaallisut; Greenlandic" : "Kalaallisut; Greenlandic",
    "Central Khmer" : "Central Khmer",
    "Kannada" : "Kannada",
    "Korean" : "Κορεάτικα",
    "Kanuri" : "Kanuri",
    "Kashmiri" : "Kashmiri",
    "Kurdish" : "Κουρδικά",
    "Komi" : "Komi",
    "Cornish" : "Cornish",
    "Kirghiz; Kyrgyz" : "Kirghiz; Kyrgyz",
    "Latin" : "Latin",
    "Luxembourgish; Lëtzeburgesch" : "Λουξεμβουργιανή γλώσσα",
    "Ganda" : "Ganda",
    "Limburgan; Limburger; Limburgish" : "Limburgan; Limburger; Limburgish",
    "Lingala" : "Lingala",
    "Lao" : "Lao",
    "Lithuanian" : "Λιθουανικά",
    "Luba-Katanga" : "Luba-Katanga",
    "Latvian" : "Λετονίας",
    "Malagasy" : "Malagasy",
    "Marshallese" : "Marshallese",
    "Maori" : "Maori",
    "Macedonian" : "Βόρειο Μακεδονικά",
    "Malayalam" : "Malayalam",
    "Mongolian" : "Mongolian",
    "Marathi" : "Marathi",
    "Malay" : "Malay",
    "Maltese" : "Maltese",
    "Burmese" : "Burmese",
    "Bokmål, Norwegian; Norwegian Bokmål" : "Bokmål, Norwegian; Norwegian Bokmål",
    "Ndebele, North; North Ndebele" : "Ndebele, North; North Ndebele",
    "Nepali" : "Nepali",
    "Ndonga" : "Ndonga",
    "Dutch; Flemish" : "Dutch; Flemish",
    "Norwegian Nynorsk; Nynorsk, Norwegian" : "Norwegian Nynorsk; Nynorsk, Norwegian",
    "Norwegian" : "Νορβηγός",
    "Ndebele, South; South Ndebele" : "Ndebele, South; South Ndebele",
    "Navajo; Navaho" : "Navajo; Navaho",
    "Chichewa; Chewa; Nyanja" : "Chichewa; Chewa; Nyanja",
    "Occitan (post 1500)" : "Occitan (post 1500)",
    "Ojibwa" : "Ojibwa",
    "Oromo" : "Oromo",
    "Oriya" : "Oriya",
    "Ossetian; Ossetic" : "Ossetian; Ossetic",
    "Panjabi; Punjabi" : "Panjabi; Punjabi",
    "Pali" : "Pali",
    "Polish" : "Πολωνός",
    "Pushto; Pashto" : "Pushto; Pashto",
    "Portuguese" : "Πορτογαλικά",
    "Quechua" : "Quechua",
    "Romansh" : "Romansh",
    "Rundi" : "Rundi",
    "Romanian; Moldavian; Moldovan" : "Romanian; Moldavian; Moldovan",
    "Russian" : "Russian",
    "Kinyarwanda" : "Kinyarwanda",
    "Sanskrit" : "Sanskrit",
    "Sardinian" : "Σαρδηνίας",
    "Sindhi" : "Sindhi",
    "Northern Sami" : "Northern Sami",
    "Sango" : "Sango",
    "Sinhala; Sinhalese" : "Sinhala; Sinhalese",
    "Slovak" : "Slovak",
    "Slovenian" : "Slovenian",
    "Samoan" : "Samoan",
    "Shona" : "Shona",
    "Somali" : "Somali",
    "Albanian" : "Αλβανικά",
    "Serbian" : "Serbian",
    "Swati" : "Swati",
    "Sotho, Southern" : "Sotho, Southern",
    "Sundanese" : "Sundanese",
    "Swedish" : "Swedish",
    "Swahili" : "Swahili",
    "Tamil" : "Tamil",
    "Telugu" : "Telugu",
    "Tajik" : "Tajik",
    "Thai" : "Thai",
    "Tigrinya" : "Tigrinya",
    "Turkmen" : "Turkmen",
    "Tagalog" : "Tagalog",
    "Tswana" : "Tswana",
    "Tonga (Tonga Islands)" : "Tonga (Tonga Islands)",
    "Turkish" : "Τούρκος",
    "Tsonga" : "Tsonga",
    "Tatar" : "Tatar",
    "Twi" : "Twi",
    "Tahitian" : "Tahitian",
    "Uighur; Uyghur" : "Uighur; Uyghur",
    "Ukrainian" : "Ουκρανικά",
    "Urdu" : "Urdu",
    "Uzbek" : "Uzbek",
    "Venda" : "Venda",
    "Vietnamese" : "Vietnamese",
    "Volapük" : "Volapük",
    "Walloon" : "Walloon",
    "Wolof" : "Wolof",
    "Xhosa" : "Xhosa",
    "Yiddish" : "Yiddish",
    "Yoruba" : "Yoruba",
    "Zhuang; Chuang" : "Zhuang; Chuang",
    "Chinese" : "Κινέζικα",
    "Zulu" : "Zulu",
    "Terms of service" : "Όροι χρήσης",
    "Terms of service have been modified!" : "Οι όροι χρήσης έχουν τροποποιηθεί!",
    "You have to accept the newest version of the terms of service in order to be able to use this service." : "Πρέπει να αποδεχθείτε την νεότερη έκδοση των όρων χρήσης για την χρήση της υπηρεσίας.",
    "Requires users to accept the terms of service before accessing data." : "Απαιτείται από τους χρήστες να αποδεχθούν τους όρους χρήσης πρίν την πρόσβαση στα δεδομένα.",
    "Requires users to accept the terms of service before accessing data.\nThis product includes GeoLite2 data created by MaxMind, available from [maxmind.com](http://www.maxmind.com)" : "Απαιτείται από τους χρήστες να αποδεχθούν τους όρους χρήσης πρίν την πρόσβαση στα δεδομένα.\nΠεριλαμβάνονται δεδομένα GeoLite2 δημιουργίας MaxMind, διαθέσιμα από [maxmind.com](http://www.maxmind.com)",
    "Loading …" : "Φόρτωση …",
    "Reset signatories & notify users" : "Επαναφέρετε τους υπογράφοντες και ειδοποιήστε τους χρήστες",
    "Save terms" : "Αποθήκευση όρων",
    "Ensure that all fields are filled" : "Βεβαιωθείτε ότι έχουν συμπληρωθεί όλα τα πεδία",
    "Terms saved successfully!" : "Οι όροι αποθηκεύτηκαν επιτυχώς!",
    "All signatories reset!" : "Επαναφορά όλων των υπογραφόντων!",
    "Require users to accept the terms of service before accessing the service." : "Απαιτείται από τους χρήστες να αποδεχθούν τους όρους χρήσης πρίν την πρόσβαση στην υπηρεσία.",
    "Show for logged-in users" : "Εμφάνιση σε συνδεδεμένους χρήστες",
    "Show on public shares" : "Εμφάνιση σε δημόσια κοινοποίηση",
    "Enter or update terms of service below." : "Εισαγάγετε ή ενημερώστε τους όρους παροχής υπηρεσιών παρακάτω.",
    "Select a region" : "Επιλογή περιοχής",
    "Select a language" : "Επιλογή γλώσσας",
    "By using this service …" : "Χρησιμοποιώντας την υπηρεσία …",
    "For formatting purposes Markdown is supported." : "Για σκοπούς μορφοποίησης υποστηρίζεται το Markdown.",
    "Saving the terms will update the text but will not send a notification to users. Notifications are only sent if you reset the signatories." : "Η αποθήκευση των όρων θα ενημερώσει το κείμενο, αλλά δεν θα στείλει ειδοποίηση στους χρήστες. Οι ειδοποιήσεις αποστέλλονται μόνο εάν επαναφέρετε τους υπογράφοντες.",
    "Existing terms of service" : "Υπάρχοντες όροι υπηρεσίας",
    "We recommend resetting the signatories if legal changes were applied. For minor changes like fixing typos or correcting links, it could be left out, as it would otherwise require all users to accept the Terms of Service again." : "Συνιστούμε να επαναφέρετε τους υπογράφοντες εάν εφαρμόστηκαν νομικές αλλαγές. Για μικρές αλλαγές, όπως η διόρθωση τυπογραφικών σφαλμάτων ή η διόρθωση συνδέσμων, θα μπορούσε να παραλειφθεί, καθώς διαφορετικά θα απαιτούσε από όλους τους χρήστες να αποδεχτούν ξανά τους Όρους Παροχής Υπηρεσιών.",
    "I acknowledge that I have read and agree to the above terms of service" : "Επιβεβαιώνω ότι έχω διαβάσει και συμφωνώ με τους παραπάνω όρους χρήσης υπηρεσίας",
    "Edit language {language} for region {country}" : "Επεξεργασία γλώσσας {language} για την περιοχή {country}",
    "Deleting …" : "Γίνεται διαγραφή …",
    "Delete language {language} for region {country}" : "Διαγραφή γλώσσας {language} για την περιοχή {country}"
},
"nplurals=2; plural=(n != 1);");
